function [x1_k, x2_k] = totvl1_optimizer(u0,v0,im1,im2,maxIter,lambda,tol)

tau   = 1/sqrt(512)/lambda;
sigma = 1/sqrt(512)/lambda;
theta = 1;
...
[m,n]=size(im1);
y11_k=zeros(m,n);  y12_k=zeros(m,n); y13_k=zeros(m,n); y14_k=zeros(m,n);
y21_k=zeros(m,n);  y22_k=zeros(m,n); y23_k=zeros(m,n); y24_k=zeros(m,n);
...
for i = 1 : maxIter
    ...
    tmp11 = y11_k + lambda*sigma*Fx(Bx(Fx(x1ba_k)));
    tmp12 = y12_k + lambda*sigma*Bx(Fx(Fy(x1ba_k)));
    tmp13 = y13_k + lambda*sigma*By(Fx(Fy(x1ba_k)));
    tmp14 = y14_k + lambda*sigma*Fy(By(Fy(x1ba_k)));
    tmp21 = y21_k + lambda*sigma*Fx(Bx(Fx(x2ba_k)));
    tmp22 = y22_k + lambda*sigma*Bx(Fx(Fy(x2ba_k)));
    tmp23 = y23_k + lambda*sigma*By(Fx(Fy(x2ba_k)));
    tmp24 = y24_k + lambda*sigma*Fy(By(Fy(x2ba_k)));
    tmp = max(sqrt(tmp11.^2+3*tmp12.^2+3*tmp13.^2+tmp14.^2+tmp21.^2+3*tmp22.^2+3*tmp23.^2+tmp24.^2),1);
    y11_k1 = tmp11./tmp;
    y12_k1 = tmp12./tmp;
    y13_k1 = tmp13./tmp;
    y14_k1 = tmp14./tmp;
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    tmp1 = x1_k + lambda*tau*div(y11_k1, y12_k1, y13_k1, y14_k1);
    tmp2 = x2_k + lambda*tau*div(y21_k1, y22_k1, y23_k1, y24_k1);
...
function f = div(a,b,c,d)
f = Bx(Fx(Bx(a)))+3*By(Bx(Fx(b)))+3*By(Bx(Fy(c)))+By(Fy(By(d)));
