function [u,v] = fotvl1_optimizer(u0,v0,im1,im2,maxIter,lambda,tol)

theta_1 = 1e2; % convergnece parameter affected by lambda
...
[m,n]=size(im1);
b11=zeros(m,n); b12=zeros(m,n); b13=zeros(m,n); b14=zeros(m,n); b15=zeros(m,n);
b21=zeros(m,n); b22=zeros(m,n); b23=zeros(m,n); b24=zeros(m,n); b25=zeros(m,n);
w11=zeros(m,n); w12=zeros(m,n); w13=zeros(m,n); w14=zeros(m,n); w15=zeros(m,n);
w21=zeros(m,n); w22=zeros(m,n); w23=zeros(m,n); w24=zeros(m,n); w25=zeros(m,n);
...
for i = 1 : maxIter
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % update u1 using DCT
    div_w_b=Bx(Fx(Bx(Fx(w11-b11))))+4*By(Bx(Fx(Bx(w12-b12))))+6*By(Fy(Bx(Fx(w13-b13))))+4*By(Fy(By(Bx(w14-b14))))+By(Fy(By(Fy(w15-b15))));
    g=theta_2*(u+d1)+theta_1*div_w_b;
    Wx=(mirt_idctn(mirt_dctn(g)./(theta_2+16*theta_1*G.^4)));
    
    % update u2 using DCT
    div_w_b=Bx(Fx(Bx(Fx(w21-b21))))+4*By(Bx(Fx(Bx(w22-b22))))+6*By(Fy(Bx(Fx(w23-b23))))+4*By(Fy(By(Bx(w24-b24))))+By(Fy(By(Fy(w25-b25))));
    g=theta_2*(v+d2)+theta_1*div_w_b;
    Wy=(mirt_idctn(mirt_dctn(g)./(theta_2+16*theta_1*G.^4)));
    ...
    abs_c=sqrt(c11.^2+4*c12.^2+6*c13.^2+4*c14.^2+c15.^2+c21.^2+4*c22.^2+6*c23.^2+4*c24.^2+c25.^2+eps);
    ...
    w15=max(abs_c-lambda/theta_1,0).*c15./abs_c;
    ...
    w25=max(abs_c-lambda/theta_1,0).*c25./abs_c;
