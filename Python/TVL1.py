...
def direct_dct_solver(u, d, w1, w2, b1, b2, G, theta_w, theta_tv, solver='fft'):
    
    div_w_b = Bx(w1-b1) + By(w2-b2)
    g = theta_w*(u+d) - theta_tv*div_w_b
    
    if solver == 'dct':
        W = dct.idct_2d(dct.dct_2d(g) / (theta_w + theta_tv*G))
    else:
        W = torch.irfft(torch.div(torch.rfft(g, 2, onesided=False), theta_w + theta_tv*G), 2, onesided=False)
    
    return W


def vector_soft_threshold(Wx, Wy, b11, b12, b21, b22, lmbda, theta_tv):
    ...
    abs_c = torch.sqrt(c11**2 + c12**2 + c21**2 + c22**2 + 2.220446049250313e-16)
    ...
    return w11, w12, w21, w22, c11, c12, c21, c22 


def tvl1_admm_optimizer(im1, im2, disp, lmbda=0.1, theta_w=.1, theta_tv=1, 
                        tol=1e-3, maxIter=500, alpha=1.8, acc=True,
    ...
    for i in range(maxIter):
        ...
        #######################################################################
        Wx = direct_dct_solver(u, d1, w11, w12, b11, b12, G, theta_w, theta_tv, solver)
        Wy = direct_dct_solver(v, d2, w21, w22, b21, b22, G, theta_w, theta_tv, solver)
 
        #######################################################################
        w11, w12, w21, w22, c11, c12, c21, c22 = vector_soft_threshold(Wx, Wy, b11, b12, b21, b22, lmbda, theta_tv)
           
        #######################################################################
        b11, b12, b21, b22, d1, d2 = c11-w11, c12-w12, c21-w21, c22-w22, d1+u-Wx, d2+v-Wy
