...
def direct_dct_solver(u, d, w1, w2, w3, w4, b1, b2, b3, b4, G, theta_w, theta_tv, solver='fft'):
    
    div_w_b = Bx(Fx(Bx(w1-b1))) + 3*By(Bx(Fx(w2-b2))) + 3*By(Bx(Fy(w3-b3))) + By(Fy(By(w4-b4)))
    
    g = theta_w*(u+d) - theta_tv*div_w_b
    
    if solver == 'dct':
        W = dct.idct_2d(dct.dct_2d(g) / (theta_w + theta_tv*G**3))
    else:
        W = torch.irfft(torch.div(torch.rfft(g, 2, onesided=False), theta_w + theta_tv*G**3), 2, onesided=False)
    
    return W


def vector_soft_threshold(Wx, Wy, b11, b12, b13, b14, b21, b22, b23, b24, lmbda, theta_tv):
    ...
    abs_c = torch.sqrt(c11**2 + 3*c12**2 + 3*c13**2 + c14**2 + c21**2 + 3*c22**2 + 3*c23**2 + c24**2 + 2.220446049250313e-16)
    ...
    return w11, w12, w13, w14, w21, w22, w23, w24, c11, c12, c13, c14, c21, c22, c23, c24


def totvl1_admm_optimizer(im1, im2, disp, lmbda=0.1, theta_w=1, theta_tv=10, 
    ...
    for i in range(maxIter):
        ...
        #######################################################################
        Wx = direct_dct_solver(u, d1, w11, w12, w13, w14, b11, b12, b13, b14, G, theta_w, theta_tv, solver)
        Wy = direct_dct_solver(v, d2, w21, w22, w23, w24, b21, b22, b23, b24, G, theta_w, theta_tv, solver)
 
        #######################################################################
        w11, w12, w13, w14, w21, w22, w23, w24, c11, c12, c13, c14, c21, c22, c23, c24 = vector_soft_threshold(Wx, Wy, b11, b12, b13, b14, b21, b22, b23, b24, lmbda, theta_tv)
           
        #######################################################################
        b11, b12, b13, b14, b21, b22, b23, b24, d1, d2 = c11-w11, c12-w12, c13-w13, c14-w14, c21-w21, c22-w22, c23-w23, c24-w24, d1+u-Wx, d2+v-Wy
