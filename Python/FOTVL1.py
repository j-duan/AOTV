...
def direct_dct_solver(u, d, w1, w2, w3, w4, w5, b1, b2, b3, b4, b5, G, theta_w, theta_tv, solver='fft'):
    
    div_w_b = Bx(Fx(Bx(Fx(w1-b1))))+4*By(Bx(Fx(Bx(w2-b2))))+6*By(Fy(Bx(Fx(w3-b3))))+4*By(Fy(By(Bx(w4-b4))))+By(Fy(By(Fy(w5-b5))))
    
    g = theta_w*(u+d) + theta_tv*div_w_b
    
    if solver == 'dct':
        W = dct.idct_2d(dct.dct_2d(g) / (theta_w + theta_tv*G**4))
    else:
        W = torch.irfft(torch.div(torch.rfft(g, 2, onesided=False), theta_w + theta_tv*G**4), 2, onesided=False)
    
    return W


def vector_soft_threshold(Wx, Wy, b11, b12, b13, b14, b15, b21, b22, b23, b24, b25, lmbda, theta_tv):
       
    c11 = Bx(Fx(Bx(Fx(Wx)))) + b11
    c12 = Fx(Bx(Fx(Fy(Wx)))) + b12
    c13 = Bx(Fx(By(Fy(Wx)))) + b13
    c14 = Fx(Fy(By(Fy(Wx)))) + b14
    c15 = By(Fy(By(Fy(Wx)))) + b15
    
    c21 = Bx(Fx(Bx(Fx(Wy)))) + b21
    c22 = Fx(Bx(Fx(Fy(Wy)))) + b22
    c23 = Bx(Fx(By(Fy(Wy)))) + b23
    c24 = Fx(Fy(By(Fy(Wy)))) + b24
    c25 = By(Fy(By(Fy(Wy)))) + b25
      
    abs_c = torch.sqrt(c11**2 + 4*c12**2 + 6*c13**2 + 4*c14**2 + c15**2 + c21**2 + 4*c22**2 + 6*c23**2 + 4*c24**2 + c25**2 + 2.220446049250313e-16)
    ...
    w14 = torch.clamp(abs_c - lmbda/theta_tv, min=0) * c14 / abs_c
    w15 = torch.clamp(abs_c - lmbda/theta_tv, min=0) * c15 / abs_c
    ...
    w24 = torch.clamp(abs_c - lmbda/theta_tv, min=0) * c24 / abs_c
    w25 = torch.clamp(abs_c - lmbda/theta_tv, min=0) * c25 / abs_c
    
    return w11, w12, w13, w14, w15, w21, w22, w23, w24, w25, c11, c12, c13, c14, c15, c21, c22, c23, c24, c25


def fotvl1_admm_optimizer(im1, im2, disp, lmbda=0.1, theta_w=1, theta_tv=100, 
    ...
    b14 = torch.zeros(N, H, W, device=torch.device(device))
    b15 = torch.zeros(N, H, W, device=torch.device(device))
    ...
    b24 = torch.zeros(N, H, W, device=torch.device(device))
    b25 = torch.zeros(N, H, W, device=torch.device(device))
    ...
    w14 = torch.zeros(N, H, W, device=torch.device(device))
    w15 = torch.zeros(N, H, W, device=torch.device(device))
    ...
    w24 = torch.zeros(N, H, W, device=torch.device(device))
    w25 = torch.zeros(N, H, W, device=torch.device(device))
    ...
    for i in range(maxIter):
        ...
        #######################################################################
        Wx = direct_dct_solver(u, d1, w11, w12, w13, w14, w15, b11, b12, b13, b14, b15, G, theta_w, theta_tv, solver)
        Wy = direct_dct_solver(v, d2, w21, w22, w23, w24, w25, b21, b22, b23, b24, b25, G, theta_w, theta_tv, solver)
 
        #######################################################################
        w11, w12, w13, w14, w15, w21, w22, w23, w24, w25, c11, c12, c13, c14, c15, c21, c22, c23, c24, c25 = vector_soft_threshold(Wx, Wy, b11, b12, b13, b14, b15, b21, b22, b23, b24, b25, lmbda, theta_tv)
           
        #######################################################################
        b11, b12, b13, b14, b15, b21, b22, b23, b24, b25, d1, d2 = c11-w11, c12-w12, c13-w13, c14-w14, c15-w15, c21-w21, c22-w22, c23-w23, c24-w24, c25-w25, d1+u-Wx, d2+v-Wy
